package MySQL.POSsupport;

import java.awt.*;

public final class ColoringConstants {
    public static Color backgroud = new Color(43, 43, 43);
    public static Color foreground = new Color(204, 204, 204);
    public static Color border = new Color(115,115,115);
    public static Color commitForeground = new Color(44, 255, 13);
    public static Color commitBackground = new Color(28,151,12, 190);
    public static Color textFieldsBackground = new Color(204,204,204);
    public static Color textFieldsForeground = new Color(43,43,43);

}

